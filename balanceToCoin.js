...
async function main() {
  ...
  const executionStatus = txResult.status ?? txResult.effects?.status ?? result?.status ?? result?.effects?.status;
  ...
  console.log('Digest:', txResult.digest ?? result.digest ?? txResult.effects?.transactionDigest);

  if (status !== 'success') {
    console.error('Transaction did not succeed — check the digest above on a Sui explorer for details.');
    if (executionStatus?.error) {
      console.error('Execution error:', JSON.stringify(executionStatus.error, null, 2));
    }
    process.exit(1);
  }

  const createdCoin = (txResult.objectChanges ?? result.objectChanges)?.find(
